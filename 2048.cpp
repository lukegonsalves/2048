...
bool check_sequence(const std::vector<int>& v){
    // write your code here
    for(int i=1; i<=v.size(); i++){ // since numbers must be > 0, i starts = 1
        if(mfind(i, v) == -1){
            return false;
        }
    }
...
void get_row(int r, const std::vector<int>& in, std::vector<int>& out){
    // write your code here
    int rows = std::sqrt(in.size()); // total number of rows
    for(int col = 0; col < rows; col++){
        int element = twod_to_oned(r, col, rows);
        out.push_back(in[element]);
    }
}
...
void get_col(int c, const std::vector<int>& in, std::vector<int>& out){
    // write your code here
    int columns = std::sqrt(in.size()); // total number of columns
    for(int row = 0; row < columns; row++){
        int element = twod_to_oned(row, c, columns);
        out.push_back(in[element]);
    }
}
...
void get_subsq(int subs, const std::vector<int>& in, std::vector<int>& out){
    ...
    int rootN = std::sqrt(n);
    // int quotient = subs / rootN;
    // std::cout << quotient;
    // int firstDigit = rootN * (subs * (quotient * rootN ));
    for(int j=0; j<rootN; j++){
        for(int i=0; i<rootN; i++){
            int row = j + ((subs/rootN) * rootN);
            int col = i + ((subs%rootN) * rootN);
            int element = twod_to_oned(row, col, n);
            out.push_back(in[element]);
        }
    }
}
